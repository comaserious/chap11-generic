...
public class Application02 {
    ...
    public static void main(String[] args) {
        ...
        /*필기.
        *  외일드카드(WildCard)
        *  제네릭 클래스타입의 객체를 메소드의 매개변수로 받을때
        *  그 객체의 타입변수를 제한할 수 있다
        *  <?> : 제한없음
        *  <? extends type> : 와일드카드 상한제한(type과 type의 후손을 이용해 생성한 인스턴스만 가능)
        *  <? super type> : 와일드카드 하한제한(type 과 type의 부모를 이용해 생성한 인스턴스만 가능)*/
...
//        wildCardFarm.anyType(new RabbitFarm<Reptile>(new Reptile()));
        ...
        /*Bunny 이거나 Bunny 후손들만 매개변수로 사용 가능하다*/
//        wildCardFarm.extendsType(new RabbitFarm<Rabbit>(new Rabbit()));
        wildCardFarm.extendsType(new RabbitFarm<Bunny>(new Bunny()));
        wildCardFarm.extendsType(new RabbitFarm<DrunkenBunny>(new DrunkenBunny()));

        /*Bunny 이거나 Bunny 부모들만 매겨변수로 사용 가능하다*/
